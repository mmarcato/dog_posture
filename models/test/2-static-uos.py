...
# define all features 
feat = df.columns[:-6]
# select rows with static features only
df_static = df[df.Type == 'static']
print(df_static.shape)
print(df_static.Position.value_counts(normalize = True))

# prepare dataframe for evaluation: select features, label,
#   cv strategy (group = dogs, stractified folds labels proportion)
X = df_static.loc[:, feat]
y = df_static.loc[:, 'Position'].values
groups = df_static.loc[:,'Dog']
...
gs_path = os.path.join(dir_current, '2-static-SKB-RF-uos.pkl')
print(gs_path)
