...
# define all features 
feat = df.columns[:-6]
# select rows without Shake features only
df_normal = df[df.Shake == 1]
print(df.shape)
print(df_normal.Position.value_counts(normalize = True))

# prepare dataframe for evaluation: select features, label,
#   cv strategy (group = dogs, stractified folds labels proportion)
X = df_normal.loc[:, feat]
y = df_normal.loc[:, 'Position'].values
groups = df_normal.loc[:,'Dog']
...
pipe = Pipeline([ 
    ...
    ('ovr', SMOTE(random_state = 0)),
    ('und', RandomUnderSampler(random_state = 0)),
...
gs_path = os.path.join(dir_current, '3-normal-SKB-RF-1.pkl')
print(gs_path)
