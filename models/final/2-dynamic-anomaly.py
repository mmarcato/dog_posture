...
df_dynamic = df[df.Type == 'dynamic']
print(df_dynamic.shape)
print(df_dynamic.Position.value_counts(normalize = True))

# prepare dataframe for evaluation: select features, label,
#   cv strategy (group = dogs, stractified folds labels proportion)
X = df_dynamic.loc[:, feat]
y = df_dynamic.loc[:, 'Shake'].values
groups = df_dynamic.loc[:,'Dog']
...
gs_path = os.path.join(dir_current, '2-dynamic-SKB-IF.pkl')
print(gs_path)
